...
export async function setCashierController(req, res, next) {
  try {
    const result = await setCashierService(req.body.email);
    return res.status(200).json({
      message: "Successfully assigned account to cashier",
      data: result,
    });
  } catch (error) {
    next(error);
